...
| <a name="input_client_read_attributes"></a> [client\_read\_attributes](#input\_client\_read\_attributes) | List of user pool attributes the application client can read from | `list(string)` | <pre>[<br>  "address",<br>  "birthdate",<br>  "email",<br>  "email_verified",<br>  "family_name",<br>  "gender",<br>  "given_name",<br>  "locale",<br>  "middle_name",<br>  "name",<br>  "nickname",<br>  "phone_number",<br>  "phone_number_verified",<br>  "picture",<br>  "preferred_username",<br>  "profile",<br>  "updated_at",<br>  "website",<br>  "zoneinfo"<br>]</pre> | no |
...
| <a name="input_client_write_attributes"></a> [client\_write\_attributes](#input\_client\_write\_attributes) | List of user pool attributes the application client can write to | `list(string)` | <pre>[<br>  "address",<br>  "birthdate",<br>  "email",<br>  "family_name",<br>  "gender",<br>  "given_name",<br>  "locale",<br>  "middle_name",<br>  "name",<br>  "nickname",<br>  "phone_number",<br>  "picture",<br>  "preferred_username",<br>  "profile",<br>  "updated_at",<br>  "website",<br>  "zoneinfo"<br>]</pre> | no |
...
| <a name="input_identity_pool_allow_unauthenticated_identities"></a> [identity\_pool\_allow\_unauthenticated\_identities](#input\_identity\_pool\_allow\_unauthenticated\_identities) | Set to true if the identity pool supports unauthenticated logins. Default is false. | `bool` | `false` | no |
| <a name="input_identity_pool_cognito_identity_providers"></a> [identity\_pool\_cognito\_identity\_providers](#input\_identity\_pool\_cognito\_identity\_providers) | (Optional) - An array of additional Amazon Cognito Identity user pools and their client IDs.<br>The properties for the additional Amazon Cognito Identity user pools are as follows:<br>- client\_id (Optional) - The client ID for the Amazon Cognito Identity User Pool.<br>- provider\_name (Optional) - The provider name for an Amazon Cognito Identity User Pool.<br>- server\_side\_token\_check (Optional) - Whether server-side token validation is enabled for the identity provider’s token or not. | <pre>list(object({<br>    client_id               = optional(string)<br>    provider_name           = optional(string)<br>    server_side_token_check = optional(bool)<br>  }))</pre> | `[]` | no |
| <a name="input_identity_pool_developer_provider_name"></a> [identity\_pool\_developer\_provider\_name](#input\_identity\_pool\_developer\_provider\_name) | The 'domain' by which Cognito will refer to your users. This name acts as a placeholder that allows your backend and the Cognito service to communicate about the developer provider. | `string` | `null` | no |
| <a name="input_identity_pool_name"></a> [identity\_pool\_name](#input\_identity\_pool\_name) | Identity pool name. If not provided, the name will be generated from the context | `string` | `null` | no |
| <a name="input_identity_pool_oidc_provider_arns"></a> [identity\_pool\_oidc\_provider\_arns](#input\_identity\_pool\_oidc\_provider\_arns) | Set of OpendID Connect provider ARNs. | `list(string)` | `[]` | no |
| <a name="input_identity_pool_saml_provider_arns"></a> [identity\_pool\_saml\_provider\_arns](#input\_identity\_pool\_saml\_provider\_arns) | An array of Amazon Resource Names (ARNs) of the SAML provider for your identity. | `list(string)` | `[]` | no |
| <a name="input_identity_pool_supported_login_providers"></a> [identity\_pool\_supported\_login\_providers](#input\_identity\_pool\_supported\_login\_providers) | A map of identity provider names to provider app IDs. | `map(string)` | `{}` | no |
...
| <a name="output_identity_pool_arn"></a> [identity\_pool\_arn](#output\_identity\_pool\_arn) | The ARN of the Identity Pool |
